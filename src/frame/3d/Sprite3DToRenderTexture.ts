...
export class Sprite3DToRenderTexture {
    ...
    constructor(sp: Sprite, orthographicVerticalSize: number = 50) {
        // super(sp.width, sp.height, Laya.RenderTargetFormat.R16G16B16A16, Laya.RenderTargetFormat.DEPTHSTENCIL_24_8, false, 4);
        ...
        let renderTexture = new RenderTexture(sp.width, sp.height, Laya.RenderTargetFormat.R8G8B8A8, Laya.RenderTargetFormat.DEPTHSTENCIL_24_8, false, 4);
        _camera.renderTarget = renderTexture;
        this.renderTexture = renderTexture;
        //再将离屏3D画到2D节点上，至此，就完成把3D画到2D的基础渲染流程
        sp.texture = new Texture(_camera.renderTarget);

        this.modePos.x = sp.width / 2;
        this.modePos.y = sp.height / 2;
    }
    ...
    set active(value: boolean) {
        this.scene3d.visible = value;
        this.model && (this.model.active = value);
    }
